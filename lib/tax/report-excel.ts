...
export function generateTaxExcelReport(result: ComparisonResult, profile: EmployeeProfile, employeeName: string): Uint8Array {
  ...
  const ctcRows: (string | number)[][] = [
    ...
    ["Variable Pay", result.ctc.variablePay],
    ["Bonus", result.ctc.bonus],
    ["Dearness Allowance", result.ctc.dearnessAllowance],
    ["Other Allowance", result.ctc.otherAllowance],
    ["Reimbursements (total)", result.ctc.reimbursementsTotal],
    ...result.ctc.reimbursements.filter((r) => r.enabled).map((r) => [`  — ${r.label}`, r.annualAmount]),
    ...
    ["Superannuation", result.ctc.superannuation],
    ["Other employer cost", result.ctc.otherEmployerCost],
